...
main_path <- '/home/laulab/Documents/Calvin' 
data_path <- file.path(main_path, 'Data')
save_path <- file.path('/mnt/hdd2/Calvin/Save/Tissue_Pairs')
#save_path <- file.path(main_path, 'Save/Tissue_Pairs')

file <- c("Correlation_data.txt", "Correlation_results.csv", "Ssec_results.csv")

#Read files
print("Reading in PLIER model ...")
PLIER_model <- readRDS(file.path(data_path, 'GTExv8VSTcomBATNoSecretome_canonicalPathwaysBloodCellSVM_multiplier_v2.Rds'))
print("Reading in GTEx data ...")
GTEx_data <- readRDS(file.path(data_path, 'gtexv8_vst_combat3_dds.Rds'))
source(file.path(main_path, 'plier_util.R'))


gsub_pattern <- 'GTEX-([A-Z0-9]*)-.*$'
gsub_replacement <- "\\1"
bonferroni_threshold = 0.05 #Only one test was done
...
data_annotation <- colData(GTEx_data) %>%
  as.data.frame()
...
for(source_tissue in all_tissues){
  for(target_tissue in all_tissues){
    ...
    if(source_tissue != target_tissue){
      ...
      #Save folder items
      save_folder <- file.path(save_path, paste0(source_tissue,"_to_",target_tissue,"_secretomics"))
...
#      source.data <- source.data[-caret::nearZeroVar(t(source.data)), ]
      ...
      p_scores_list <- rowSums(-log(correlation_matrix_p + 1e-10))
      r_scores_list <- rowSums(-log(abs(correlation_matrix_r)))
...
run_time <- difftime(time_0, time_fine, unit = "hours")
